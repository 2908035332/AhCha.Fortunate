...
namespace AhCha.Fortunate.Repositories.SqlSugar
{
    ...
    public partial class SqlSugarRepository<TEntity> where TEntity : class, new()
    {
        private readonly string[] UpdateIgnoreColumns = new string[] { "CreateTime", "CreateUserId" };
        ...
        public virtual async Task<ISugarQueryable<TEntity>> Union(List<ISugarQueryable<TEntity>> listQuery)
        {
            if (listQuery == null || listQuery.Count < 2)
            {
                // 处理异常情况，至少需要两个查询对象才能进行合并操作
                throw new ArgumentException("At least two query objects are required for union operation.");
            }

            var resultQuery = listQuery[0];  // 将第一个查询对象作为初始值

            for (int i = 1; i < listQuery.Count; i++)
            {
                resultQuery = _db.Union(resultQuery, listQuery[i]);  // 依次与后续的查询对象进行合并
            }
